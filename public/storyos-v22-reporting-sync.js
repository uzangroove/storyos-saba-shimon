(() => {
  ...
  const OPERATOR_REPORT_URL='/operator-report-v2235.html?v='+VERSION;
  let lastAdminSig='';
  let syncing=false;
  ...
  function openOperatorReport(){location.href=OPERATOR_REPORT_URL}
  function installButton(){
    if($('#v2234OperatorBtn'))return;
    const actions=$('.head-actions');if(!actions)return;
    const b=document.createElement('button');
    b.id='v2234OperatorBtn';b.type='button';b.className='btn';b.textContent='📱 דיווח מפעיל';
    b.title='פתיחת אפליקציית דיווח כניסה/יציאה ומשוב';
    b.onclick=openOperatorReport;
  ...
  window.StoryOSReportingSync={version:VERSION,hydrate:hydrateShared,push:pushAdmin,pull:pullOperator,openOperatorReport,adminPath:ADMIN_PATH,operatorPath:OP_PATH};
})();