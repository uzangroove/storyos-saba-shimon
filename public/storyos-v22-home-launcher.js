(() => {
  ...
  function build(){if($('#v2218Home'))return;addStyles();const root=document.createElement('section');root.id='v2218Home';root.setAttribute('aria-label','מסך ראשי StoryOS');root.innerHTML=`<div class="v2218-wrap"><div class="v2218-brand"><img class="v2218-logo" id="v2218BrandLogo" data-firebase-path="${BRAND}" src="${direct(BRAND)}" alt="סבא שמעון ומוריס"><h1>StoryOS · סבא שמעון</h1><p>בחר פעילות או גן כדי להתחיל</p></div><div class="v2218-grid">${items.map(tileMarkup).join('')}</div><div class="v2218-footer">גרסה ${VERSION} · מסך כניסה נקי</div></div>`;document.body.appendChild(root);
