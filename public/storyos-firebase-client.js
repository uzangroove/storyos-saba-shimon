(() => {
  ...
  async function sendReset(email){ await ready; return state.modules.authMod.sendPasswordResetEmail(state.auth,email); }
  ...
  async function signInWithPrompt(){
    ...
    return new Promise((resolve,reject)=>{
      ...
      modal.querySelector('[data-reset]').onclick=async()=>{const email=String(new FormData(form).get('email')||'').trim();if(!email){err.textContent='יש להזין קודם כתובת דוא״ל.';return;}err.textContent='שולח קישור לאיפוס סיסמה…';try{await sendReset(email);err.style.color='#246b42';err.textContent='נשלח קישור לאיפוס הסיסמה. פתח את המייל, קבע סיסמה חדשה וחזור להתחבר.';}catch(ex){err.style.color='#a33';err.textContent='לא ניתן לשלוח איפוס: '+(ex.code||ex.message||ex);}};
      form.onsubmit=async e=>{e.preventDefault();const data=new FormData(form),email=String(data.get('email')||'').trim(),password=String(data.get('password')||'');err.style.color='#a33';err.textContent='מתחבר…';try{const user=await signIn(email,password);closeLogin();resolve(user);}catch(ex){err.textContent='ההתחברות נכשלה: '+(ex.code||ex.message||ex);}};
    });
  }
  ...
  const api={version:'22.8.2',config:{...firebaseConfig,apiKey:'[client-configured]'},ready:ensureReady,currentUser,tokenInfo,signIn,sendReset,signInWithPrompt,signOut,listFiles,getUrl,upload};
