(() => {
  ...
  function scheduleDates(startDate,count){
    const d=new Date(startDate+'T12:00:00'),dow=d.getDay(),day=dow===0?'ראשון':dow===2?'שלישי':'';
    const rows=window.StoryOSSchoolCalendar?.rows||[];
    if(day&&rows.length){const dates=rows.filter(r=>r.day===day&&r.status!=='סגור'&&r.date>=startDate).slice(0,count).map(r=>r.date);if(dates.length)return dates;}
    return Array.from({length:count},(_,i)=>addDays(startDate,i*7));
  }
  ...
  function renderWeekly(){
    ...
    body.innerHTML=`<div class="v22-admin-grid"><section class="v22-admin-panel"><h3>יצירת מסלול שבועי שנתי</h3><form id="v2214WeeklyForm" class="v22-admin-form"><label>גן<select name="garden" required><option value="">בחר גן</option>${gardenOpts}</select></label><label>פעילות<select name="program">${programOpts}</select></label><label>תאריך מפגש ראשון<input name="startDate" type="date" required></label><label>שעה<input name="time" type="time" value="09:00"></label><label>מספר מפגשים<input name="count" type="number" min="1" max="60" value="37"></label><label>משך<input name="duration" value="45 דקות"></label><label class="wide">כותרת בסיס<input name="title" value="מפגש שבועי"></label><label class="wide">הערות<textarea name="notes" placeholder="למשל: פעילות שבועית לכל שנת הלימודים"></textarea></label><div class="v22-admin-form-actions"><button class="btn primary" type="submit">צור מסלול ופתח את כל המפגשים בלוח</button></div></form><p style="color:var(--muted);margin-top:10px">בימי ראשון ושלישי המערכת משתמשת בלוח שנת הלימודים ומדלגת אוטומטית על ימים שבהם הגן סגור.</p></section><section class="v22-admin-panel"><h3>מסלולים קיימים (${weekly.length})</h3>${weekly.length?`<div class="v22-admin-list">${weekly.map(w=>`<div class="v22-admin-row"><div><b>${esc(w.garden)} · ${esc(w.program||'ללא שיוך פעילות')}</b><small>${esc(w.startDate||w.day||'')} · ${esc(w.time)} · ${w.count} מפגשים · ${esc(w.duration)}</small></div><div class="v22-admin-row-actions"><button class="btn small" data-open-calendar="${w.id}">לוח</button><button class="btn small danger" data-delete-weekly="${w.id}">מחיקה</button></div></div>`).join('')}</div>`:'<div class="v22-empty">עדיין לא הוגדר מסלול שבועי.</div>'}</section></div>`;
    const form=$('#v2214WeeklyForm');if(form)form.onsubmit=e=>{e.preventDefault();const f=new FormData(form);const garden=String(f.get('garden')||'').trim();if(!garden)return;const record={id:uid(),garden,program:String(f.get('program')||''),startDate:String(f.get('startDate')||''),time:String(f.get('time')||'09:00'),count:Math.max(1,Number(f.get('count')||37)),duration:String(f.get('duration')||'45 דקות'),title:String(f.get('title')||'מפגש שבועי').trim(),notes:String(f.get('notes')||'').trim(),createdAt:Date.now()};const arr=loadWeekly();arr.push(record);saveWeekly(arr);const adminNow=loadAdmin();const dates=scheduleDates(record.startDate,record.count);dates.forEach((date,i)=>{adminNow.events.push({id:`${record.id}_${i+1}`,date,time:record.time,garden:record.garden,program:record.program,title:`${record.title} ${i+1}`,status:'planned',duration:record.duration,notes:record.notes,scheduleId:record.id,meetingNo:i+1});});saveAdmin(adminNow);renderWeekly();};
  ...
  window.StoryOSWeeklyManagement={version:VERSION,render:renderWeekly,scheduleDates};
})();
